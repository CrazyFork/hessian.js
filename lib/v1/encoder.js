...
proto.writeString = function (str) {
  ...
  while (length > 0x8000) {
    ...
    this.byteBuffer
      .put(0x73) // 's'
      .putUInt16(sublen) // string 的length, 非bytes
...
proto._writeHashMap = function (obj) {
  ...
  // :bm, so a es6 map can be traversed in  ordered
  if (supportES6Map && obj instanceof Map) {
    ...
    for (var i = 0; i < keys.length; i++) {
      var k = keys[i];
      this.writeString(k);  //:bm, writeString 内部每个string都记录了对应的length. 所以能分隔开key, value
      this.write(obj[k]);
    }
  }
...
proto.writeObject = function (obj) {
  ...
  if (!obj.$class || !obj.$) {
    ...
    // :bm, so a well know map class type is eased from writing
 ...
 * @return {this}
 */
// :todo, 为啥还要支持序列化格式? 因为encoder默认传入的根节点就是 js-to-java 格式化后的
...
proto.write = function (val) {
  ...
  // java.lang.Object
  // so a Date object is been described as `java.lang.Object`
  if (utils.isJavaObject(val.$class)) {
    // a date or non-complex object
